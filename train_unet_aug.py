...
class MRI2D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply TorchIO augmentation to image only
        if self.augment and self.aug_transform is not None:
            img, mask = self.aug_transform(img, mask)
...
def main():
    ...
    # Optional: load existing checkpoint if readable
    existing_ckpt = "checkpoints/best_unet.pt"
    if os.path.exists(existing_ckpt):
        try:
            model.load_state_dict(torch.load(existing_ckpt, map_location=device))
            print(f"Loaded existing checkpoint from {existing_ckpt}")
        except Exception as e:
            print(f"Could not load {existing_ckpt}: {e}")

    criterion = nn.BCEWithLogitsLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

    # Save to a folder you own
    ...
    for epoch in range(EPOCHS):
        ...
        if val_iou > best_iou:
            best_iou = val_iou
            torch.save(model.state_dict(), "checkpoints_aug/best_unet_aug.pt")
            print("Saved best model to checkpoints_aug/best_unet_aug.pt")
