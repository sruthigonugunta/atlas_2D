...
# =========================
def main():
    img_dir = "data/dataset_2D_jsonsplit/test/images"
    mask_dir = "data/dataset_2D_jsonsplit/test/masks"
    ckpt = "checkpoints/best_unet.pt"

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("Device:", device)

    ds = MRI2D(img_dir, mask_dir)

    model = UNet().to(device)
    model.load_state_dict(torch.load(ckpt, map_location=device))
    model.eval()

    os.makedirs("prediction_figures", exist_ok=True)

    # prendre 5 slices avec lésion
    chosen = []
    ...
    with torch.no_grad():
        for k, idx in enumerate(chosen):
            ...
            plt.tight_layout()
            out = os.path.join("prediction_figures", f"pred_{k}_{name}")
